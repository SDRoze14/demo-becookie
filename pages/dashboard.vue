<template>
  <div>
    ...
    <div class="flex items-center py-3 border-b border-gray-100">
      ...
      <div class="ml-auto">
        <input
          class="px-2 py-1 text-primary bg-blue-200 rounded-md"
...
export default {
  ...
  methods: {
    ...
    async fetchReport() {
      ...
      await this.$api.getConsentLogs(params).then((response) => {
        if (response.data.entities.length > 0)
          this.more = response.data.page_information.next_page_id;
    ...
    async fetchDailyReport() {
      ...
      self.xaxis_of_month.forEach(async (x, i) => {
        res.push({
          id: x,
          data_reject: 0,
          data_reconsent: 0,
      ...
      res.forEach((e) => {
        data.forEach((d) => {
          if (d.id == e.id) {
            ...
            e.data_reconsent =
              d.change_to_partial_count + d.change_to_all_count;
    ...
    exportPDFClick() {
      ...
      domtoimage
        ...
        .then(function (dataUrl) {
          ...
          const filename = domain_name + "-report-" + now + ".pdf";
          doc.save(filename);
        })
